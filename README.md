...
While a turn runs, the answer streams into an ephemeral draft bubble with tool activity and a native Stop button that aborts the turn. Turn-end summaries carry a token and cost footer, summaries with tables or code fences render as native rich messages, and the agent can push screenshots and other artifacts to the chat as photos, albums, or documents. `/status` reports what each session is doing without disturbing it (inside a forum thread it reaches only that thread's session), retries, model fallbacks, and context compaction announce themselves, and both commands sit in the bot's menu button.
