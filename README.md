...
Parallel sessions share one bot: each message carries a session badge, replies route to the session that sent the replied-to message, and titles name the tmux window. If the target Telegram group has forum topics enabled, each session gets its own thread instead.
...
While a turn runs, the answer streams into an ephemeral draft bubble with tool activity and a native Stop button that aborts the turn. Turn-end summaries carry a token and cost footer, summaries with tables or code fences render as native rich messages, and the agent can push screenshots and other artifacts to the chat as photos, albums, or documents. A finished (green) summary carries a red button that closes the session and its tmux tab. `/status` reports what each session is doing without disturbing it (inside a forum thread it reaches only that thread's session), `/fleet` lists every omp tmux window with its live state read from the pane titles, retries, model fallbacks, and context compaction announce themselves, and all commands sit in the bot's menu button.
