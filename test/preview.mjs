...
writeFileSync(join(root, "notify-telegram.json"), JSON.stringify({ token: "0".repeat(30), chatId: 1, offset: 1, quietSeconds: 0 }));
...
const ctx = {
  ...
  sessionManager: { getSessionId: () => "01a00000-0000-0000-0000-000000000000", getSessionName: () => "Fix FTS5 rebuild" },
  ui: { onTerminalInput: () => () => {} },
  setInterval: () => 0,
  setTimeout: () => 0,
  clearTimer() {},
...
tools.get("ask").execute("p", {
  context: "`rebuild_index` drops the table first, so a crash mid-run leaves no index:\n```rust\nconn.execute(\"DELETE FROM idx\")?;\n```",
  questions: [{
    id: "fix",
    ...
    options: [
      ...
      { label: "Build beside, then swap", description: "Doubles peak disk use.", preview: "ALTER TABLE idx_new RENAME TO idx;" },
      { label: "Leave it", description: "Nightly job retries anyway.", discouraged: true },
    ],
    recommended: 1,
  }],
}, undefined, undefined, { ...ctx, invokeTool: () => new Promise(() => {}) });
await new Promise((r) => setTimeout(r, 200));

await tools.get("notify_status").execute("s", {
