...
const inboxCount = (id) => (existsSync(inboxOf(id)) ? readdirSync(inboxOf(id)).filter((f) => f.endsWith(".json")).length : 0);
...
const singleQuestion = {
  questions: [{ id: "backend", question: "Which backend?", options: [{ label: "SQLite" }, { label: "Postgres" }], recommended: 0 }],
};
...
check("poller lock acquired", JSON.parse(readFileSync(join(root, "notify-telegram/poller.lock"), "utf8")).sessionId === one.id);
check("timers registered even though topic creation failed", one.timers.length === 2);
check("topic failure is logged rather than swallowed", one.warns.some((w) => JSON.stringify(w.meta ?? {}).includes("TOPICS_DISABLED")));
...
check("tags are five lowercase alphanumerics", [one.id, two.id].every((id) => /^[a-z0-9]{5}$/.test(record(id).tag)));
...
const stateA = {};
const runSingle = two.tools.get("ask").execute("c1", singleQuestion, undefined, undefined, stubbornCtx(two.ctx, stateA));
await settle(150);
...
api.queued = [{ update_id: 100, callback_query: { id: "cb1", data: pick, from: { id: CHAT }, message: { message_id: 7, chat: { id: CHAT } } } }];
await one.pump(250);
...
await settle(150);
const typeButton = lastCall("sendMessage").body.reply_markup.inline_keyboard.flat().find((b) => b.text === "Type an answer").callback_data;
...
const multi = { questions: [{ id: "t", question: "Targets?", multi: true, options: [{ label: "linux" }, { label: "macos" }, { label: "windows" }] }] };
const stateC = {};
...
for (const payload of [`o:${askId}:0:0`, `o:${askId}:0:2`, `d:${askId}:0`]) {
  writeFileSync(join(inboxOf(two.id), `${300 + Math.random()}.json`), JSON.stringify({ kind: "callback", value: payload }));
  await two.pump(120);
}
...
const localResult = await two.tools
  .get("ask")
  .execute("c6", singleQuestion, undefined, undefined, { ...two.ctx, invokeTool: async () => ({ content: [{ type: "text", text: "SQLite" }], details: {} }) });
...
await two.pump(120);
check("free text with no pending ask becomes a steer", two.steers.length === 1 && two.steers[0].options.deliverAs === "steer");
...
check("the rejection is logged", one.warns.some((w) => w.m.includes("unexpected chat")));

api.queued = [
  { update_id: 111, callback_query: { id: "cbX", data: `o:${tag2}-1:0:0`, from: { id: STRANGER }, message: { message_id: 7, chat: { id: CHAT } } } },
];
await one.pump(200);
check("a press from a foreign sender is dropped", inboxCount(two.id) === 0);
check("a foreign press is not acknowledged", !called("answerCallbackQuery").some((c) => c.body.callback_query_id === "cbX"));
check("the foreign origin is logged", one.warns.some((w) => w.m.includes("unexpected origin")));
check("allowed_updates asks for both kinds", lastCall("getUpdates").body.allowed_updates.join(",") === "message,callback_query");
check("offset advances past rejected updates", JSON.parse(readFileSync(join(root, "notify-telegram.json"), "utf8")).offset === 112);

// ------------------------------------------------------------------- rendering
heading("telegram formatting");
const rich = {
  questions: [{ id: "q", question: "Rename `foo` to **bar**?", options: [{ label: "yes", description: "Touches `src/lib.rs`", preview: "fn foo() {}" }] }],
...
check("angle brackets inside code are escaped", !html.text.includes("fn foo() -> u8 { 3 }") && html.text.includes("-&gt; u8"));
...
check("the rejection is logged", two.warns.some((w) => w.m.includes("rich send rejected")));
...
await two.tools.get("session_badge").execute("c8", { emoji: "\u{1F9EA}", label: "index work" }, undefined, undefined, two.ctx);
...
writeFileSync(
  ...
  JSON.stringify({ pid: 999999, tag: "zzzzz", name: "", topicId: null, topicName: "", cwd: "/x", emoji: "\u{1F41D}", label: "", lastNotified: 0, heartbeat: Date.now() - 600_000 }),
);
...
check("topic name carries badge and folder", /^.+ sqlitegis \u00B7 /u.test(called("createForumTopic").at(-2).body.name));
await alpha.fire("session_stop");
await settle();
check("messages go into the topic", lastCall("sendMessage").body.message_thread_id === topicAlpha);
check("badge head is dropped inside a topic", lastCall("sendMessage").body.text.startsWith("<b>\u{1F7E2} Turn finished</b>"));
...
api.queued = [{ update_id: 120, message: { message_id: 30, date: 1, chat: { id: CHAT }, message_thread_id: topicBeta, text: "use duckdb" } }];
await one.pump(250);
...
api.queued = [{ update_id: 121, message: { message_id: 31, date: 1, chat: { id: CHAT }, message_thread_id: 424242, text: "orphan" } }];
await one.pump(250);
...
check("topic deleted on clean exit", called("deleteForumTopic").some((c) => c.body.message_thread_id === topicAlpha));
check("topic queued in case the process dies first", JSON.parse(readFileSync(join(root, "notify-telegram/pending-topics.json"), "utf8")).includes(topicAlpha));
...
check("the next start sweeps the delete queue", JSON.parse(readFileSync(join(root, "notify-telegram/pending-topics.json"), "utf8")).length === 0);
...
await settle();
check("falls back to a flat message when topics are unavailable", lastCall("sendMessage").body.message_thread_id === undefined);
...
check("each session records the ids it sent", record(rr1.id).recent.includes(rr1Id) && record(rr2.id).recent.includes(rr2Id));
check("recorded ids do not overlap", !record(rr1.id).recent.includes(rr2Id));

api.queued = [
  { update_id: 200, message: { message_id: 90, date: 1, chat: { id: CHAT }, text: "do the other thing", reply_to_message: { message_id: rr1Id } } },
];
await one.pump(250);
...
api.queued = [
  { update_id: 202, message: { message_id: 92, date: 1, chat: { id: CHAT }, text: "stale", reply_to_message: { message_id: 999999 } } },
];
...
check("context is prepended to the first question so the terminal shows it", seen.params.questions[0].question.startsWith("The rebuild takes 40 minutes"));
...
writeFileSync(join(inboxOf(ctxSession.id), "800.json"), JSON.stringify({ kind: "callback", value: `o:${ctxAskId}:0:0` }));
await ctxSession.pump(150);
writeFileSync(join(inboxOf(ctxSession.id), "801.json"), JSON.stringify({ kind: "callback", value: `o:${ctxAskId}:1:0` }));
await ctxSession.pump(150);
check("the two-question ask still completes", (await ctxRun).details.results.length === 2);
check("tool description names the supported markdown subset", ctxSession.tools.get("ask").description.includes("triple-backtick fences") && ctxSession.tools.get("ask").description.includes("~~strikethrough~~"));
check("tool description no longer claims telegram-only context", !ctxSession.tools.get("ask").description.includes("only on Telegram"));
...
for (const s of contenders) {
  s.heartbeat();
  check(`heartbeat survives a missing lock file for ${s.id.slice(0, 8)}`, !s.warns.some((w) => w.m.includes("heartbeat failed")));
  break;
}
...
check("the asking session owns the poller lock for this section", JSON.parse(readFileSync(join(root, "notify-telegram/poller.lock"), "utf8")).sessionId === mq.id);
const three = {
  questions: [
...
check("position line shows one of three with a red light", lastCall("sendMessage").body.text.includes("Input needed 1 of 3"));

const press = (q, opt, id) => ({
  update_id: id,
  callback_query: { id: `cb${id}`, data: `o:${mqAsk}:${q}:${opt}`, from: { id: CHAT }, message: { message_id: 7, chat: { id: CHAT } } },
});
...
const cb = (data, id) => ({ update_id: id, callback_query: { id: `k${id}`, data, from: { id: CHAT }, message: { message_id: 7, chat: { id: CHAT } } } });
...
check("multi-select offers a Done button", mixedRows.at(-1).some((b) => b.text === "Done"));
...
check("the failure is logged instead", boom.warns.some((w) => w.m.includes("failed")));
...
check("the entry after a corrupt one still lands", boom.steers.some((x) => x.text === "still works"));
...
globalThis.fetch = async (url, init) => {
  ...
  if (method === "sendMessage" && throttled === 0) {
    throttled++;
    return { ok: false, status: 429, json: async () => ({ ok: false, description: "Too Many Requests", parameters: { retry_after: 1 } }) };
  }
...
const mdRun = mdSession.tools.get("ask").execute("md", { context: md, questions: [{ id: "q", question: "ok?", options: [{ label: "yes" }] }] }, undefined, undefined, stubbornCtx(mdSession.ctx, {}));
await settle(200);
...
check("link becomes an anchor with an escaped query", mdBody.text.includes('<a href="https://example.com/a?x=1&amp;y=2">the docs</a>'));
...
const hugeRun = mdSession.tools.get("ask").execute("hg", { context: huge, questions: [{ id: "q", question: "ok?", options: [{ label: "yes" }] }] }, undefined, undefined, stubbornCtx(mdSession.ctx, {}));
await settle(200);
...
api.queued = [{ update_id: 501, message: { message_id: 71, date: 1, chat: { id: CHAT }, photo: [{ file_id: "y" }], caption: "look at this" } }];
...
check("the telegram message is edited on cancellation", closing !== undefined && closing.body.message_id === escMessageId);
check("it says it was cancelled at the terminal", closing.body.text.includes("Cancelled at the terminal"));
check("the keyboard is explicitly cleared", Array.isArray(closing.body.reply_markup?.inline_keyboard) && closing.body.reply_markup.inline_keyboard.length === 0);
...
const walkState = {};
const walkRun = esc.tools.get("ask").execute("e3", {
  questions: [
    { id: "a", question: "First?", options: [{ label: "x" }, { label: "y" }] },
    { id: "b", question: "Second?", options: [{ label: "p" }] },
  ],
}, undefined, undefined, stubbornCtx(esc.ctx, walkState));
await settle(150);
...
check("the first question's keyboard is cleared when it is answered", firstClosed.body.reply_markup.inline_keyboard.length === 0);
...
check("the terminal sees the discouraged marker too", stanceParams.params.questions[0].options[2].description.startsWith("(discouraged)"));
check("the marker is not lost when there was no description", stanceParams.params.questions[0].options[3].description === undefined);
check("discouraged never reaches the strict native tool", stanceParams.params.questions[0].options.every((o) => !("discouraged" in o)));
check("neutral descriptions are untouched", stanceParams.params.questions[0].options[0].description === "no strong view");
check("the caller's option objects are not mutated", stanceAsk.questions[0].options[2].description === "here for contrast");
check("marker text has a single definition", stRows[1][0].text.includes("(preferable)") && stBody.text.includes("(preferable)"));
...
const bareState = {};
const bareRun = st.tools.get("ask").execute("s2", {
  questions: [{ id: "b", question: "Which?", recommended: 0, options: [{ label: "take this" }, { label: "avoid this", discouraged: true }] }],
}, undefined, undefined, stubbornCtx(st.ctx, bareState));
await settle(180);
...
sem.ctx.sessionManager.getBranch = () => [
  ...
  { type: "message", message: { role: "assistant", content: [{ type: "text", text: "Intro paragraph.\n\n" + branchTail }] } },
];
...
const semState = {};
const semRun = sem.tools.get("ask").execute("sm", { questions: [{ id: "q", question: "ok?", options: [{ label: "yes" }] }] }, undefined, undefined, stubbornCtx(sem.ctx, semState));
await settle(150);
check("a question message carries the red input-needed line", lastCall("sendMessage").body.reply_markup !== undefined && lastCall("sendMessage").body.text.includes("\u{1F534} Input needed"));
...
await settle(150);
check("a turn without a summary is blocked once", firstStop.some((r) => r?.decision === "block"));
check("the block explains what to call", firstStop.find((r) => r?.decision === "block").reason.includes("notify_status"));
check("after the block the fallback message still goes out", lastCall("sendMessage").body.text.includes("Turn finished"));

await rs.fire("input");
await rs.tools.get("notify_status").execute("n1", { summary: "Migration rewritten and **all 14 tests pass**. Nothing pending.", urgency: "green" }, undefined, undefined, rs.ctx);
...
await rs.fire("input");
await rs.tools.get("notify_status").execute("n2", { summary: "Blocked: need the staging credentials.", urgency: "red" }, undefined, undefined, rs.ctx);
await rs.fire("session_stop");
await settle(150);
check("agent-chosen red renders as action required", lastCall("sendMessage").body.text.includes("\u{1F534} Action required"));

await rs.fire("input");
await rs.tools.get("notify_status").execute("n3", { summary: "Two designs possible, opinion wanted.", urgency: "purple" }, undefined, undefined, rs.ctx);
await rs.fire("session_stop");
await settle(150);
check("an unknown urgency degrades to green", lastCall("sendMessage").body.text.includes("\u{1F7E2}"));

await rs.fire("input");
const empty = await rs.tools.get("notify_status").execute("n4", { summary: "   ", urgency: "green" }, undefined, undefined, rs.ctx);
check("an empty summary is rejected", empty.isError === true);
const blockedAgain = await rs.fire("session_stop");
check("a new turn requires a new summary", blockedAgain.some((r) => r?.decision === "block"));
...
check("summary and question are both in the body", tqMsg.text.includes("diff is large") && tqMsg.text.includes("How should we proceed?"));
...
await tq.pump(250);
check("a press starts the next turn with the label", tq.steers.some((x) => x.text === "Review the diff" && x.options === undefined));
const retired = lastCall("editMessageText");
check("the question message is retired with the choice", retired.body.text.includes("Chosen:") && retired.body.text.includes("Review the diff"));
...
await tq.pump(250);
check("a stale press is answered with a closure notice", called("sendMessage").length === beforeStale2 + 1 && lastCall("sendMessage").body.text.includes("closed"));

// A new question supersedes an old one.
await tq.fire("input");
await tq.tools.get("notify_status").execute("q2", { summary: "First.", urgency: "orange", options: ["A", "B"] }, undefined, undefined, tq.ctx);
await tq.fire("session_stop");
await settle(150);
const firstQ = lastCall("sendMessage").body.reply_markup.inline_keyboard[0][0].callback_data;
await tq.fire("input");
await tq.tools.get("notify_status").execute("q3", { summary: "Second.", urgency: "orange", options: ["C", "D"] }, undefined, undefined, tq.ctx);
await tq.fire("session_stop");
await settle(200);
check("terminal input retires the older standing question", called("editMessageText").some((c) => c.body.text.includes("Answered at the terminal")));
...
check("a press after a resume still starts the next turn", resumed.steers.some((x) => x.text === "D"));

// A terminal reply while a choice question stands on Telegram must close it.
await tq.fire("input");
await tq.tools.get("notify_status").execute("q5", { summary: "Choose.", urgency: "orange", options: ["Go on", "Halt"] }, undefined, undefined, tq.ctx);
await tq.fire("session_stop");
await settle(150);
...
check("terminal reply closes the standing telegram question", closing5.some((c) => c.body.text.includes("Answered at the terminal")));
check("its buttons are removed", closing5.some((c) => Array.isArray(c.body.reply_markup?.inline_keyboard) && c.body.reply_markup.inline_keyboard.length === 0));
...
const badOpts = await tq.tools.get("notify_status").execute("q4", { summary: "x", urgency: "green", options: ["only-one"] }, undefined, undefined, tq.ctx);
...
const bpState = {};
const bpRun = bp.tools.get("ask").execute("bp", {
  questions: [{
    id: "p",
    question: "Pick",
    options: [
...
}, undefined, undefined, stubbornCtx(bp.ctx, bpState));
await settle(180);
...
api.queued = [
  { update_id: 210, message: { message_id: 95, date: 1, chat: { id: CHAT }, text: "after the restart", reply_to_message: { message_id: rr1Id } } },
];
rmSync(join(root, "notify-telegram/poller.lock"), { force: true });
rrBack.heartbeat();
await rrBack.pump(250);
check("a reply to a pre-restart message still routes", inboxCount(rr1.id) === 1);
readdirSync(inboxOf(rr1.id)).forEach((f) => unlinkSync(join(inboxOf(rr1.id), f)));
...
const taState = {};
const taRun = ta.tools.get("ask").execute("ta", { questions: [{ id: "q", question: "ok?", options: [{ label: "yes" }] }] }, undefined, undefined, stubbornCtx(ta.ctx, taState));
await settle(180);
...
writeFileSync(
  ...
  JSON.stringify({ pid: 999999, tag: "qqqqq", name: "", topicId: null, topicName: "", cwd: "/x", emoji: "\u{1F344}", label: "", lastNotified: Date.now() + 10, recent: [], standing: null, heartbeat: Date.now() - 600_000 }),
);
